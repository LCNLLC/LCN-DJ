        ...
        <div class="col-md-4 mb-3 ml-auto" >
            <div class="bg-grad-1 text-white rounded-lg overflow-hidden">
              ...
              <div class="px-3 pt-3 pb-3">
                  <div class="h4 fw-700 text-center">{{ single_price( admin_pay_to_courier(Auth::user()->id)) }}</div>
              ...
              <table class="table aiz-table mb-0">
                  ...
                  <tbody>
                      @foreach ($courier_withdraw_requests as $key => $courier_withdraw_request)
                          <tr>
                              <td>{{ $key+1 }}</td>
                              <td>{{ date('d-m-Y', strtotime($courier_withdraw_request->created_at)) }}</td>
    ...
    <div class="modal fade" id="request_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                @if (admin_pay_to_courier(Auth::user()->id) > 5) 
                    <form class="" action="{{ route('courier.money_withdraw_request.store') }}" method="post">
                        @csrf
                        <div class="modal-body gry-bg px-3 pt-3">
                            <div class="row">
                                ...
                                <div class="col-md-9">
                                    <input type="number" lang="en" class="form-control mb-3" name="amount" min="{{ get_setting('minimum_courier_amount_withdra') }}" max="{{ admin_pay_to_courier(Auth::user()->id) }}" placeholder="{{ translate('Amount') }}" required>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-3">
