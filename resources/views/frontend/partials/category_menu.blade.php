<div class="aiz-category-menu bg-white rounded-0 border-top" id="category-sidebar" style="width:100%; max-width:270px;">
        ...
        @foreach (\App\Models\Category::where('level', 0)->orderBy('order_level', 'desc')->get()->take(9) as $key => $category)
            <li class="category-nav-element border border-top-0" data-id="{{ $category->id }}">
                <a href="{{ route('products.category', $category->slug) }}" class="text-truncate text-dark px-4 fs-14 d-block hov-column-gap-1">
                    {{--  @dd($category->slug);  --}}
                    <img class="cat-image lazyload mr-2 opacity-60 img-fluid"
