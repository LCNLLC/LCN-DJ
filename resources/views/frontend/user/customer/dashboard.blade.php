...
@section('panel_content')
    ...
    <!-- Biding product -->
     <div class="row align-items-center mb-2 mt-1">
        <div class="col-6">
            <h3 class=" mb-0 fs-14 fs-md-16 fw-700 text-dark">{{ translate('My Accepted bidlist')}}</h3>
        </div>
        <div class="col-6 text-right">
            <a class="text-blue fs-10 fs-md-12 fw-700 hov-text-primary animate-underline-primary" href="{{ route('customer.bid-products') }}">{{ translate('View All') }}</a>
        </div>
    </div>
    @php
        $user = auth()->user();
        $products = \App\Models\Product::whereHas('bids', function ($query) use ($user) {
    ...
    @if (count($products) > 0)
        <div class="row row-cols-xxl-5 row-cols-xl-4 row-cols-lg-4 row-cols-md-3 row-cols-sm-2 row-cols-2 gutters-16 border-top border-left mx-1 mx-md-0 mb-4">
            @foreach($products->take(5) as $key => $product)
                @if ($product != null)
                    <div class="aiz-card-box col py-3 text-center border-right border-bottom has-transition hov-shadow-out z-1" id="product{{ $product->id }}">
                        <div class="position-relative h-140px h-md-200px img-fit overflow-hidden mb-3">
                            ...
                            <!-- <div class="absolute-top-right aiz-p-hov-icon">
                                <a href="javascript:void(0)" onclick="removeFromWishlist({{ $product->id }})" data-toggle="tooltip" data-title="{{ translate('Remove from bidlist') }}" data-placement="left">
                            ...
                            <a class="cart-btn absolute-bottom-left w-100 h-35px aiz-p-hov-icon text-white fs-13 fw-700 d-flex justify-content-center align-items-center" 
                                href="javascript:void(0)" onclick="showAddToCartModal({{ $product->id }})">{{ translate('Add to Cart') }}</a>
                        </div>
                        <!-- Product Name -->
                        ...
                        <!-- Price -->
                        <div class="fs-14">
                            <span class="fw-600 text-primary">{{ $product->bids->first()->amount }}</span>
