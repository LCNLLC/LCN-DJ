                    ...
                    <div class="col-xl-3">
                        <div class="aiz-filter-sidebar collapse-sidebar-wrap sidebar-xl sidebar-right z-1035">
                            ...
                            <div class="collapse-sidebar c-scrollbar-light text-left">
                                ...
                                @php
                                $distinctRatings = \App\Models\Product::where('rating', '>', 0)->distinct()->pluck('rating');
                                @endphp
                                <div class="bg-white border mb-3">
                                    @if (Route::currentRouteName() != 'products.brand')
                                        <select class="form-control form-control-sm aiz-selectpicker rounded-0" data-live-search="true" name="rating" onchange="filter()">
                                            <option value="">{{ translate('Ratings')}}</option>
                                            @foreach ($distinctRatings as $rating)
                                                <option value="{{ $rating }}" @if ($product_rating == $rating) selected @endif>{{ $rating }} {{ translate('star') }}</option>
                                            @endforeach
                                        </select>
                                    @endif
                                </div>
                        ...
                        <div class="text-left">
                            <div class="row gutters-5 flex-wrap align-items-center">
                                ...
                                <div class="col-6 col-lg-auto mb-3 w-lg-200px">
                                    <select class="form-control form-control-sm aiz-selectpicker rounded-0" name="product_type" onchange="filter()">
                                        <option value="">{{ translate('Product Type')}}</option>
                                        <option value="donation" @isset($product_type) @if ($product_type == 'donation') selected @endif @endisset>{{ translate('Donation & Aid')}}</option>
                                        <option value="auction" @isset($product_type) @if ($product_type == 'auction') selected @endif @endisset>{{ translate('Used Products')}}</option>
                                        <option value="other" @isset($product_type) @if ($product_type == 'other') selected @endif @endisset>{{ translate('Other')}}</option>
                                    </select>
                                </div>
                                <div class="col-6 col-lg-auto mb-3 w-lg-200px">
                                    @if (Route::currentRouteName() != 'products.brand')
                        ...
                        <div class="px-3">
                            <div class="row gutters-16 row-cols-xxl-4 row-cols-xl-3 row-cols-lg-4 row-cols-md-3 row-cols-2 border-top border-left">
                                @foreach ($products as $key => $product)
                                    ...
                                    @else
                                    <div class="col border-right border-bottom has-transition hov-shadow-out z-1">
                                            @include('frontend.partials.product_donation',['product' => $product])
                                        </div>
                                    @endif
                                @endforeach
                            </div>
