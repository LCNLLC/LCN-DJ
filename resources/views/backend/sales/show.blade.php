             ...
             <!--    <div class="col text-md-left text-center">
                    ...
                    @if ($order->seller_id == $admin_user_id || get_setting('product_manage_by_admin') == 1)
                        ...
                        @if (get_setting('courier_commission_activation'))
                            <div class="col-md-12 ml-auto">
                                <label for="courier_id">{{ translate('Assign Courier Boy') }}</label>
                                @if (($delivery_status == 'pending' || $delivery_status == 'confirmed' || $delivery_status == 'picked_up') && auth()->user()->can('assign_delivery_boy_for_orders'))
                                    <select class="form-control aiz-selectpicker" data-live-search="true"
                                        data-minimum-results-for-search="Infinity" id="courier_id">
                                        <option value="">{{ translate('Select Courier Boy') }}</option>
                                        @foreach ($delivery_boys as $delivery_boy)
                                            <option value="{{ $delivery_boy->id }}"
                                                @if ($order->courier_id == $delivery_boy->id) selected @endif>
                                                {{ $delivery_boy->name }} from : 
                                                {{ $delivery_boy->courier->city->name }},
                                                {{ $delivery_boy->courier->state->name }},
                                                {{ $delivery_boy->courier->country->name }}  
                                                 ({{ $delivery_boy->courier->vehicle_type }})
                                            </option>
                                        @endforeach
                                    </select>
                                @else
                ...
                @endif
                    ...
                    <div class="col-md-3 ml-auto">
                           ...
                           <label for="courier_id">{{ translate('Delivery proof') }}</label>
...
@section('script')
    <script type="text/javascript">
        $('#courier_id').on('change', function() {
            ...
            var delivery_boy = $('#courier_id').val();
            $.post('{{ route('orders.delivery-boy-assign') }}', {
                _token: '{{ @csrf_token() }}',
                order_id: order_id,
                courier_id: delivery_boy
            }, function(data) {
                AIZ.plugins.notify('success', '{{ translate('Courier boy has been assigned') }}');
            });
        });
