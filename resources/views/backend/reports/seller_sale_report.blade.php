                ...
                <table class="table table-bordered aiz-table mb-0">
                    ...
                    <tbody>
                        @foreach ($sellers as $key => $seller)
                            @if($seller != null)
                                <tr>
                                    <td>{{is_null($seller->shop) ? '': $seller->shop->name}}({{is_null($seller) ? '': $seller->name}})</td>
                                    @if($seller->shop != null)
                                        <td>{{ $seller->name }}</td>
                                    @else
                                        <td>--</td>
                                    ...
                                    {{--  <td>
                                        @php
                                            $num_of_sale = 0;
                                            if (!is_null($seller->user) && !is_null($seller->user->products)) {
                                    ...
                                    <td>
                                        {{ single_price(\App\Models\OrderDetail::where('seller_id', is_null($seller->user) ? 0 : $seller->user->id)->sum('price')) }}
                                    </td>
                                </tr>
                            @endif
                        @endforeach
