        ...
        <div class="card-body">
          <form action="{{ route('couriers.update', $courier->id) }}" method="POST">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <input type="text" placeholder="{{translate('Name')}}" id="name" name="name" class="form-control" value="{{$courier->user->name}}" required>
                    </div>
                </div>
                <div class="form-group row">
                    <label class="col-sm-3 col-from-label" for="email">{{translate('Email Address')}}</label>
                    <div class="col-sm-9">
                        <input type="text" placeholder="{{translate('Email Address')}}" id="email" name="email" class="form-control" value="{{$courier->user->email}}" required>
                    </div>
                </div>
                <div class="form-group row">
