...
<div class="card">
    <form class="" id="sort_products" action="" method="GET">
            ...
            <div class="col-md-2 ml-auto">
                <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" id="user_id" name="user_id" onchange="sort_products()">
                    ...
                    <option value="{{ $seller->id }}" @if ($seller->id == $seller_id) selected @endif>
                        {{ is_null($seller->shop) ? '' : $seller->shop->name}} ({{is_null($seller) ? '' : $seller->name}})
                    </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="card-body">
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        <th data-breakpoints="md">{{translate('Bid Count')}}</th>
            ...
            <td>
                @if($type =='Seller')
               <span class="badge badge-inline badge-danger" style="padding: 12px;">{{ $product->bids_count }}</span>
                @else 
                  <a href="{{route('auction.bids',$product->id)}}"><span class="badge badge-inline badge-danger" style="padding: 12px;">{{ $product->bids_count }}</span></a>
                @endif
              
            </td>
            <td>
                @if($product->auction_product_status =='Open')
                <span class="badge badge-inline badge-success" style="padding: 12px;">{{ $product->auction_product_status }}</span>
                @elseif($product->auction_product_status =='Close')
                <span class="badge badge-inline badge-warning" style="padding: 12px;">{{ $product->auction_product_status }}</span>
                @else
                <span class="badge badge-inline badge-danger" style="padding: 12px;">{{ $product->auction_product_status }}</span>
                @endif
            ...
            @if($type !='Seller')
            <td>
                <select class="form-control" name="product_bidding_status" data-id="{{ $product->id }}" onchange="product_bid_status(this)" id="product_bidding_status" style="width: 15%;">
                    <option value="1" <?php if ($product->auction_product_status == 'Open') echo "selected"; ?>>{{translate('Open')}}</option>
                    <option value="2" <?php if ($product->auction_product_status == 'Close') echo "selected"; ?> >{{translate('Close')}}</option>
                    <option value="3" <?php if ($product->auction_product_status == 'Sold') echo "selected"; ?> >{{translate('Sold')}}</option>
               </select>
            </td>
            @endif
...
<script type="text/javascript">
  ...
  function product_bid_status(el){
    var bid_status_val = $('#product_bidding_status').val();
    var bid_product_id = $(el).data('id');
           $.post('{{ route('auction.products.bid') }}', {_token:'{{ csrf_token() }}', id:bid_product_id, status:bid_status_val}, function(data){
            if(data == 1){
                AIZ.plugins.notify('success', '{{ translate('Product bid updated successfully') }}');
            }
            else{
                AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
            }
