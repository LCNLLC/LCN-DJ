            ...
            <div class="card-body">
                <form action="{{ route('shipping_configuration.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="radio mar-btm">
                        <input id="product-shipping" class="magic-radio" type="radio" name="shipping_type" value="product_size_and_area_wise_shipping" @if(get_setting('shipping_type') == 'product_size_and_area_wise_shipping') checked @endif>
                        <label for="product-shipping">
                            <span>{{translate('Product Size and Area Wise Shipping Cost')}}</span>
                            <span></span>
                        </label>
                    </div>
                    <!-- <div class="radio mar-btm">
            ...
            <div class="card-body">
                <ul class="list-group">
                    <li class="list-group-item">
                        1. {{ translate('Product Size and Area Wise Shipping  Cost calculation: Shipping cost is calculate by addition of each product size shipping cost') }}.
                    </li>
                   <!--  <li class="list-group-item">
                        2. {{ translate('Product Wise Shipping Cost calculation: Shipping cost is calculate by addition of each product shipping cost') }}.
                    </li>
                    <li class="list-group-item">
                        3. {{ translate('Flat Rate Shipping Cost calculation: How many products a customer purchase, doesn\'t matter. Shipping cost is fixed') }}.
                    </li>
                    <li class="list-group-item">
                        4. {{ translate('Seller Wise Flat Shipping Cost calculation: Fixed rate for each seller. If customers purchase 2 product from two seller shipping cost is calculated by addition of each seller flat shipping cost') }}.
                    </li>
                    <li class="list-group-item">
                        5. {{ translate('Area Wise Flat Shipping Cost calculation: Fixed rate for each area. If customers purchase multiple products from one seller shipping cost is calculated by the customer shipping area. To configure area wise shipping cost go to ') }} <a href="{{ route('cities.index') }}">{{ translate('Shipping Cities') }}</a>.
                    </li>
                    <li class="list-group-item">
                        6. {{ translate('Carrier Based Shipping Cost calculation: Shipping cost calculate in addition with carrier. In each carrier you can set free shipping cost or can set weight range or price range shipping cost. To configure carrier based shipping cost go to ') }} <a href="{{ route('carriers.index') }}">{{ translate('Shipping Carriers') }}</a>.
                    </li> -->
                </ul>
            </div>
        </div>
