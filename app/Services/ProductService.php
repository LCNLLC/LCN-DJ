...
class ProductService
{
    public function store(array $data)
    {
        ...
        if($collection['product_type'] != 3)
        {
             $collection['auction_end_date'] = null;
        }
        else
        {
           $collection['auction_end_date'] = strtotime($collection['auction_end_date']);
        }
        ...
        if(isset($collection['colors']) and is_array($collection['colors'])){
            if (
        ...
        if (isset($combinations[0]) and count($combinations[0]) > 0) {
        ...
        if (isset($collection['choice_no']) && $collection['choice_no']) {
            ...
            foreach ($collection['choice_no'] as $key => $no) {
                ...
                if (is_array(request()[$str]) or is_object(request()[$str])) {
