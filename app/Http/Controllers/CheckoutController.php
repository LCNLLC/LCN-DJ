...
class CheckoutController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if (!$request->owner_id and User::find($request->owner_id)) {
            array_push($error, "Valid Owner Id required.");
            $flag = true;
        }

        if (!$request->payment_option || !in_array($request->payment_option, $paymentMethods)) {
            array_push($error, "Payment Option required.");
            $flag = true;
        }
        
        if (!$request->agree_checkbox) {
            array_push($error, "Click the Terms and condition.");
            $flag = true;
        }
        if($flag){
            ...
            $termsContent =  Page::where('type', 'terms_conditions_page')->first();
            session_start();

            $total = $_SESSION["total_price_order"];

            return view('frontend.payment_select', compact('carts', 'shipping_info', 'total','termsContent', 'error'));
        }

        unset($_SESSION["total_price_order"]); 
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            ...
            session_start();
            $_SESSION["total_price_order"] = $total;
            $termsContent =  Page::where('type', 'terms_conditions_page')->first();
             
            return view('frontend.payment_select', compact('carts', 'shipping_info', 'total','termsContent'));
