...
class CommissionController extends Controller
{
    ...
    public function pay_to_courier(Request $request)
    {

        $amount = admin_pay_to_courier($request->courier_id); 
        ...
        $request->session()->put('payment_type', 'courier_payment');
        $request->session()->put('payment_data', $data);


        if ($request->payment_option == 'cash') {
            return $this->courier_payment_done($request->session()->get('payment_data'), null);
        }
        elseif ($request->payment_option == 'bank_payment') {
        ...
        //redirects to this method after successfull seller payment
    public function courier_payment_done($payment_data, $payment_details){

        $courier = Courier::where('user_id', $payment_data['courier_id'])->firstOrFail();
        $courier->paid_by_admin = $courier->paid_by_admin + $payment_data['amount'];
        $courier->save();
   
        $payment = new CourierPayment;
        $payment->courier_id = $courier->user_id;
        $payment->amount = $payment_data['amount'];
        $payment->payment_method = $payment_data['payment_method'];
        $payment->txn_code = $payment_data['txn_code'];
        $payment->payment_details = $payment_details;
        ...
        if ($payment_data['payment_withdraw'] == 'withdraw_request') {
            $courier_withdraw_request = CourierWithdrawRequest::findOrFail($payment_data['withdraw_request_id']);
            $courier_withdraw_request->status = '1';
            $courier_withdraw_request->viewed = '1';
            $courier_withdraw_request->save();
        }
        ...
        if ($payment_data['payment_withdraw'] == 'withdraw_request') {
            flash(translate('Payment completed'))->success();
            return redirect()->route('courier_withdraw_requests_all');
        }
        else {
            flash(translate('Payment completed'))->success();
            return redirect()->route('couriers.index');
    ...
    public function pay_to_customer(Request $request)
    {
        ...
        $amount = admin_pay_to_customer($request->customer_id); 
        ...
        if ($request->payment_option == 'cash') {
            return $this->customer_payment_done($request->session()->get('payment_data'), null);
        }
        elseif ($request->payment_option == 'bank_payment') {
        ...
        //redirects to this method after successfull seller payment
    public function customer_payment_done($payment_data, $payment_details){

        $customer = Customer::where('user_id', $payment_data['customer_id'])->firstOrFail();
        $customer->paid_by_admin = $customer->paid_by_admin + $payment_data['amount'];
        $customer->save();
   
        $payment = new CustomerPayment;
        $payment->customer_id = $customer->user_id;
        $payment->amount = $payment_data['amount'];
        $payment->payment_method = $payment_data['payment_method'];
        $payment->txn_code = $payment_data['txn_code'];
        $payment->payment_details = $payment_details;
        ...
        if ($payment_data['payment_withdraw'] == 'withdraw_request') {
            $customer_withdraw_request = CustomerWithdrawRequest::findOrFail($payment_data['withdraw_request_id']);
            $customer_withdraw_request->status = '1';
            $customer_withdraw_request->viewed = '1';
            $customer_withdraw_request->save();
        }
        ...
        if ($payment_data['payment_withdraw'] == 'withdraw_request') {
            flash(translate('Payment completed'))->success();
            return redirect()->route('customer_withdraw_requests_all');
        }
        else {
            flash(translate('Payment completed'))->success();
            return redirect()->route('customers.payment_histories');
        }
    }
}
