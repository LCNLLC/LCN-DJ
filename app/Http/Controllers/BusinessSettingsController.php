...
class BusinessSettingsController extends Controller
{
    public function __construct() {
        ...
        $this->middleware(['permission:product_price_adjustment'])->only('product_price_adjustment');
         $this->middleware(['permission:courier_commission_configuration'])->only('courier_commission');
    }
    ...
    public function courier_commission(Request $request)
    {
        return view('backend.courier.courier_commission.index');
    }
    ...
    public function courier_commission_update(Request $request){
        foreach ($request->types as $key => $type) {
            $business_settings = BusinessSetting::where('type', $type)->first();
            if($business_settings!=null){
                $business_settings->value = $request[$type];
        ...
        flash(translate('Courier Commission updated successfully'))->success();
        return back();
    }

    public function product_price_adjustment(Request $request)
    {
        return view('backend.setup_configurations.product_price_adjustment.index');
    }

    public function product_price_adjustment_update(Request $request){
        foreach ($request->types as $key => $type) {
            $business_settings = BusinessSetting::where('type', $type)->first();
            if($business_settings!=null){
                $business_settings->value = $request[$type];
        ...
        flash(translate('Product adjustment price updated successfully'))->success();
        return back();
    }
    ...
    public function shipping_cost_adjustment(Request $request)
    {
        return view('backend.setup_configurations.shipping_cost_adjustment.index');
    }
    public function shipping_cost_adjustment_update(Request $request){
        foreach ($request->types as $key => $type) {
            $business_settings = BusinessSetting::where('type', $type)->first();
            if($business_settings!=null){
                $business_settings->value = $request[$type];
        ...
        flash(translate('Shipping cost adjustment  updated successfully'))->success();
        return back();
    }

    public function referral_withdraw_settings(Request $request){
     $gnl = BusinessSetting::whereIn('type',['affiliate_withdraw_day','affiliate_withdraw_person','affiliate_withdraw_deadline'])->get();

        foreach ($gnl as $setting) {
            if ($request->has($setting->type)) {
                $setting->value = $request->{$setting->type};
                $setting->update();
            }
        }
...
//        Artisan::call('cache:clear');
        ...
        flash(translate('Referral withdraw rules updated successfully'))->success();
        return back();
    }
