...
class HomeController extends Controller
{
   ...
   public function referred(){
     ...
     return view('frontend.refer.affiliate_rules_banner');
   }
...
//       $gn_setting = GeneralSettings::first();
       ...
       return view('frontend.refer.com_history_n', compact('ref_comms'));
   }
   ...
   public function refComAdjust(Request $request)
   {
    ...
    //    }
       ...
       $affiliate_withdraw_day=referlSetting('affiliate_withdraw_day');
       $affiliate_withdraw_person=referlSetting('affiliate_withdraw_person');
//        if ($day >= $general_setting->affiliate_withdraw_day && $user->referral_commission->count() >= $general_setting->affiliate_withdraw_person) {

       if ($day >= $affiliate_withdraw_day && $user->referral_commission->count() >= $affiliate_withdraw_person) {
...
//                'interest_balance' => bcadd($user->interest_balance, $ref_comm->amount),
               'ref_cm_balance' => bcadd($user->ref_cm_balance, $ref_comm->amount),
           ]);
       ...
       return redirect()->route('user.referral.com')->withSuccess(__('affiliate.Your referral commission') .' '. $ref_comm->amount . __('affiliate.has been adjusted with your  balance.'));
   }
    ...
    public function index()
    {
        ...
        $featured_categories = Cache::rememberForever('featured_categories', function () {
            return Category::where('featured', 1)->inRandomOrder()->get();
        });

        $todays_deal_products = Cache::rememberForever('todays_deal_products', function () {
            return filter_products(Product::where('published', 1)->where('todays_deal', '1'))->inRandomOrder()->get();
        });

       $newest_products = Cache::remember('newest_products', 3600, function () {
            return filter_products(Product::latest()->limit(12)->get());
        });
         $newest_products = Product::latest()->limit(12)->get();
         $newest_products = $newest_products->shuffle();
         $today = Carbon::now();
        $sliders = Slider::where('status',1)->orderBy('position', 'asc')->get();


       $boosting_products = Product::where('boosting_status', 1)
        ->where('digital', 0)
       ...
       $donation_products = Product::where('product_type', 2)
        ->where('digital', 0)
        ->where('auction_product', 0)
        ->where('wholesale_product', 0)
        ->where('approved', 1)
        ...
        $auction_products = Product::where('product_type', 3)
        ->where('digital', 0)
        ->where('auction_product', 0)
        ->where('wholesale_product', 0)
        ->where('approved', 1)
        ...
        return view('frontend.index', compact('featured_categories', 'todays_deal_products', 'newest_products','boosting_products','donation_products','auction_products','sliders','today', 'lang'));
    }
    ...
    public function login()
    {
        ...
        }else if(Route::currentRouteName() == 'courier.login' && get_setting('courier_login_page_bg') == 1){
            return view('frontend.courier_login');
        }
    ...
    public function dashboard()
    {
        ...
        } elseif (Auth::user()->user_type == 'courier') {
    ...
    public function profile(Request $request)
    {
        if (Auth::user()->user_type == 'seller') {
            ...
            return view('frontend.user.profile',compact('langs'));
        }
    }
    ...
    public function jobProfile(Request $request)
    {
       ...
       // return $job_profile->skills;
        return view('frontend.user.job_profile',compact('job_profile'));
    }
    ...
    public function jobProfileSave(Request $request)
    {
        ...
        $merged_data = array_merge($job_profile_data, $json_data);

// Check if 'id' is present in the request
        if ($request->has('id')) {
    // Update the existing record
            JobProfile::where('id', $request->id)->update($merged_data);
        } else {
    // Create a new record
            JobProfile::create($merged_data);
        }

        flash(translate('Job Profile has done successfully'))->success();
        return redirect()->route('job.profile');
    ...
    public function userProfileUpdate(Request $request)
    {
        ...
        if($customer){
            $customer->cash = $request->cash_status;
            $customer->bank_payment_status = $request->bank_payment_status;
            $customer->bank_name = $request->bank_name;
            $customer->bank_acc_name = $request->bank_acc_name;
            $customer->bank_acc_no = $request->bank_acc_no;
            $customer->bank_routing_no = $request->bank_routing_no;
    ...
    public function product(Request $request, $slug)
    {
        ...
        if(($detailedProduct->boosting_status  == 'Active') && ($detailedProduct->boosting_type  == 'click'))
        {
            $product = Product::findOrFail($detailedProduct->id);
            if( ($detailedProduct->click + 1) >=  $detailedProduct->boosting_click)
            {
                $product->click   = '';
                $product->boosting_click   = '';
                $product->boosting_status   = 2;
            }
            else
            {
                $cookie_id = request()->cookie('my_cookie'.$detailedProduct->id);
                if($cookie_id != $detailedProduct->id )
                {
                    $product->click   = ($detailedProduct->click + 1);
                    Cookie::queue('my_cookie'.$detailedProduct->id, $detailedProduct->id, 10080);
...
public function faq(Request $request)
{
    ...
    $page =  Page::where('type', 'faq_page')->first();
    $faqs = FAQ::where('status',1)->where('lang',$locale)->orderBy('artical_code')->paginate(10);
    return view("frontend.policies.faq", compact('faqs','page'));
}
